...
<%
	// 내가쓴글 세부사항 보기
	Comm_BoardVO cvo = (Comm_BoardVO)request.getAttribute("MyBoardDetail");
	// 내가쓴글 글종류 카테고리
...
$(function() {
	...
	$('.ComList').on('click', function() {
		location.href="<%= request.getContextPath()%>/board/MyComboardList.do?stuId=<%= cvo.getStu_id()%>"
	})
	
	
	$('.updateCom').on('click', function(){
		vcomform = $('#ComForm').serialize();
// 		vcomform += "&Comm_gu = " + $('#selectgu option:selected').val().trim();
		$.ajax({
			url : "<%=request.getContextPath() %>/myboard/MyboardUpdate.do?cNo=<%= cvo.getComm_no()%>" ,
			data : vcomform ,
			type : 'post',
			success : function(res) {
				if(res.flag != "실패"){
				alert('글이 수정되었습니다.')
				location.href="<%= request.getContextPath()%>/myboard/MyboardDetail.do?cNo=<%= cvo.getComm_no()%>"
				}
			},
			error : function(xhr){
				alert("오류 : " + xhr.status);
	...
	$('.Delete').on('click', function() {
			...
			$.ajax({
				url : '<%=request.getContextPath() %>/myboard/MyboardDelete.do',
        ...
        <div class="row">
            <div class="row col-md-12">
                <div class="frame content">
                    <div class="stumanage">
	    	 		<div class="title stuttl">내가쓴글 상세보기</div>
...
%>	
	...
	<label>조회수 : </label><span class="Cnsspan">&nbsp;<%= cvo.getComm_rc() %></span><br>
	<label>글종류 : </label><span class="Cnsspan">&nbsp;<%= guvo.getComm_nm() %></span><br>
	<label>작성내용 : </label><hr>
	<p><%= cvo.getComm_cn() %></p>
	</form>
